...
fprintf(result, '航速下限（节）：%g\n',Frmin);
fprintf(result, '航速上限（节）：%g\n',Frmax);
...
for i=1:n
    L = M16shipL_T(x(i,2))*k;
    B = L / M16shipLB_T(x(i,2));
    S = M16shipS_T(x(i,2))*k*k;
    T = B/M16shipBT_T(x(i,2));
    Cb = M16shipCb_T(x(i,2));
